...
class Utilisateur
{
    ...
    public static function delete(int $ID_utilisateur)
    {
        try {
            // Création de l'objet PDO pour la connexion à la BDD
            $db = new PDO('mysql:host=localhost;dbname=' . DB_NAME, DB_USER, DB_PASS);
            ...
            // On relie les paramètres à nos marqueurs nominatifs à l'aide d'un bindValue 
            $query->bindValue(':ID_utilisateur', htmlspecialchars($ID_utilisateur), PDO::PARAM_INT);
    ...
    public static function update(int $ID_utilisateur, string $user_lastname, string $user_firstname, string $user_email, string $user_password)
    {
        try {
            // Création de l'objet PDO pour la connexion à la BDD
            $db = new PDO('mysql:host=localhost;dbname=' . DB_NAME, DB_USER, DB_PASS);
            ...
            $sql = "UPDATE `utilisateur` SET `user_lastname` = :user_lastname, `user_firstname` = :user_firstname, `user_email` = :user_email, `user_password` = :user_password WHERE `ID_utilisateur` = :ID_utilisateur";
